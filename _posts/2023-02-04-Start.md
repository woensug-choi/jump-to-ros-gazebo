...
윈도우 시작버튼을 오른쪽 클릭해 Windows PowerShell(관리자)[더보기](https://fast-drizzle-23f.notion.site/Powershell-29563e91dbc84e848b364157ff68185f) 실행을 클릭, 윈도우 터미널을 관리자 모드로 실행하고 다음 명령어를 입력한 다음 컴퓨터를 다시 시작한다. 이때 WSL의 설치와 함께 리눅스 운영체제 중 하나인 Ubuntu의 22.04 버전을 함께 설치하며, 설치과정에서 Ubuntu에서 사용 할 비밀번호를 설정하게 된다.

```bash
wsl --install Ubuntu-22.04
```
실행이 안될 때 아래의 명령어로 실행해 봅시다.
```bash
